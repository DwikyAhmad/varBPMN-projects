...
public class MybankResourceImpl extends MybankResourceComponent {
    ...
    public Map<String, Object> transfer(VMJExchange vmjExchange) {
		...
		if (!((hasTaskState(processes, "put status FAIL to res") || (hasTaskState(processes, "put status FAIL to res") || hasTaskState(processes, "init int accumulate as withdraw + finalAmount"))))) {
		    response.put("status", "FAIL");
		...
		if (!response.containsKey("status") || !response.get("status").equals("FAIL")) {
		    ...
		    processService.upsert(new ProcessInstance(processid, "init int balanceTarget as account.get(balance)"));
